...
## 阶段说明
+ JavaScript基础--->Web API--->JavaScript高级

+ 提示
  + 1.敲代码用指法,指法配图，坚持一周者即可熟练
  + 2.可以使用金山打字通，此处并非打广告,个人建议，天天敲代码，不停的反复的敲练习和作业，速度和基本功的快速提升
...
- 注意点
  ...
  ***1、引用外部js文件的script标签中不可以写JavaScript代码，***
  ...
  2、script标签放在body的标签的最后，有时放在head中。放在header中后面body中的代码可能执行不了***
...
```
最小值：Number.MIN_VALUE，这个值为： 5e-324   不要用小数验证小数
...
- 数值判断
  ...
  - NaN：not a number
    - NaN 与任何值都不相等，包括他本身
  - isNaN: is not a number    不是数字--true   是数字--false
...
3. undefined:变量已声明未赋值；函数没有明确返回值，如果接收了，结果也是undefined
4. undefined+数值=NaN
5. 该对象存储的是一个null时，是一个对象 ，typeof（null）输出不是null
...
### 三元运算符
	...
	是对if……else语句的一种简化写法
	表达式1为true 执行表达式2 并把结果给变量
	表达式2为true 执行表达式2 并把结果给变量
...
```javascript
求整数1～100的累加值，但要求碰到个位为3的数则停止累加  break
...
> 所谓数组，就是将多个元素（通常是同一类型，类型可以不一样）按一定顺序排列放到一个集合中，那么这个集合我们就称之为数组。
### 数组的定义
...
//通过构造函数创建数组
    ...
    var 数组名=new Array(长度); //写一个数字，表示长度，默认元每个数据是undefined   
    var 数组名=new Array(多个值); //表示数据长度，就是数据的个数
...
返回值详解：
    如果函数没有显示的使用 return语句 ，但是在调用时接收了，那么函数有默认的返回值：undefined
...
> JavaScript中，arguments对象是比较特别的一个对象，实际上是当前函数的一个内置属性。也就是说**所有函数都内置了一个arguments对象**，arguments对象中存储了传递的所有的实参。arguments是一个**伪数组**，因此及可以进行遍历
...
function f1(person) {
  ...
  console.log(p);//{name: "ls", age: 18, salary: 1000}
  console.log(person);//{name: "aa", age: 18, salary: 10}
}
...
console.log(num);//报错,只是函数中的形参，不是全局变量
```
## 内置对象
...
```javascript
// 1 栈操作(先进后出)
push(值)   //值追加到数组中，加到最后，返回值是数组的长度；
pop() 		//删除数组中最后一个元素，返回值是删除的值。取出数组中的最后一项，修改length属性
// 2 队列操作(先进先出)
push()
shift()		//删除数组中第一个元素，返回值是删除的值。  取出数组中的第一个元素，修改length属性
unshift(值) 	//在数组最前面插入一个新的元素，返回数组的长度
...
// 4 操作方法
concat(数组1，数组2,.......)  	//把参数拼接到当前数组
slice(开始的索引，结束的索引) 	//从当前数组中截取一个新的数组，不影响原来的数组，参数start从0开始,end从1开始，不包括结束的索引对应的元素值
splice(开始的位置，要删除的个数，要替换的值)	//删除或替换当前数组的某些项目，参数start, deleteCount, options(要替换的项目)
// 5 位置方法
indexOf(元素值，位置)、lastIndexOf()   //如果没找到返回-1
// 6 迭代方法 不会修改原数组(可选)
every()--一假即假、filter()--过滤、forEach()--循环、map()--映射、some()--一真即真
// 7 方法将数组的所有元素连接到一个字符串中。
join("字符串")-----返回的是字符串

every(function(元素的值ele，索引index，原来的数组array 没用){
  
})----返回值是布尔值。如果这个数组中的每个元素的值都符合条件，才返回true


filter(function(元素的值ele，索引index，原来的数组array 没用){
  
})-----返回的是数组中每个元素都符合条件的，组成了一个新的数组。
map(函数)---数组中的每个元素都要执行这个函数，把执行后的结果重新的全部的放在一个新的数组中


forEach(function(元素的值ele，索引index，原来的数组array 没用){
...
function clear() {
  ...
  for (var i = 0; i < array.length; i++) {
    ...
    if (o[item]) {
      o[item]++;
        console.log(o[item]);
        console.log(o);//最后一次循环：{c: 1, a: 3, z: 1, x: 1}
    }else{
      o[item] = 1;
    }
  }
...
str[0]   		//HTML5，IE8+支持 和charAt()等效  例：'h234s'[0]
// 2 字符串操作方法
